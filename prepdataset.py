...
def prepdataset(CEI_file,MWR_file,z_max=2000,predictors = ['BT','T'], dt_common=30, dz_common=40, interpMethod='linear', verbose=False, outputDir="datasets/", saveNetcdf=False,storeImages=False,fmtImages='.png'):
	...
	#temperature[:,0]=np.nan		# First level of radiometer is bad
	...
	if storeImages:
		#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
		plt.figure()
		plt.title("Original temperature | "+t_mwr[0].strftime("%Y/%m/%d"))
		...
		plt.savefig(outputDir+"T_orig_"+t_mwr[0].strftime("%Y%m%d")+fmtImages)
		#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
		plt.figure()
		plt.title("Original backscatter | "+t_cei[0].strftime("%Y/%m/%d"))
		...
		plt.ylabel("Alt (m agl)")
		#plt.show(block=False)
		plt.savefig(outputDir+"BT_orig"+t_cei[0].strftime("%Y%m%d")+fmtImages)
		#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
if __name__=='__main__':
	...
	n_invalid=prepdataset(CEI_dir+CEI_file,MWR_dir+MWR_file, saveNetcdf=True)
