...
def cluster2Dview_multi(variable1,varname1,variable2,varname2,zoneIDs,storeImages=False,fmtImages=".svg",figureDir=""):
	'''Plots the projection of the clusters onto the space generated by
	two predictors. It can be used to visualize clusters (boundary layer classification).
	...
	plt.show(block=False)
	if storeImages:
		plt.savefig(figureDir+"multi_cluster2Dview_"+varname1+'-'+varname2+fmtImages)
	#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
def clusterZTview_multi(t_values,z_values,zoneIDs,delete_mask=None,storeImages=False,fmtImages=".svg",figureDir=""):
	'''Plots cluster labels (areas) against time and height (boundary layer classification).
	...
	plt.show(block=False)
	if storeImages:
		plt.savefig(figureDir+"multi_clusterZTview"+fmtImages)
	#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
